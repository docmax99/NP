...
export default function BookingCard({ house, userId, Ankunft, Abgang,GästeZahl }) {
  ...
  // Berechnung des Gesamtpreises basierend auf Nächten und Hauskosten (könnte dynamischer gemacht werden)
  const totalPrice = houseData.Kosten ? houseData.Kosten * nights + 45 + 108 - 150 : 0;
  ...
  return (
    <div className="flex flex-col max-w-md mx-auto bg-white rounded-md shadow-md p-6 sticky top-10 border border-gray-200">
      ...
      <PriceBreakdown price={houseData.Kosten || 0} nights={nights} />
    </div>
  );
}
